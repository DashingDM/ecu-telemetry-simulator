...
def fbox(ax, x, y, w, h, color, label, fontsize=9, text_color='white', style="round,pad=0.1"):
    box = FancyBboxPatch((x, y), w, h, boxstyle=style, facecolor=color, edgecolor='white', linewidth=1.5)
    ax.add_patch(box)
    ax.text(x+w/2, y+h/2, label, ha='center', va='center', fontsize=fontsize,
            color=text_color, fontweight='bold', wrap=True)

def outline(ax, x, y, w, h, color, label, fontsize=11):
...
fbox(ax, 3.5, 5.3, 2.9, 1.3, '#1a5276', "POSIX Queues\n(sensor_q)\n(telemetry_q)", fontsize=8)
fbox(ax, 3.5, 3.6, 2.9, 1.4, '#117a65', "CLI Interface\n(stdin/stdout)\nstatus|tasks|heap", fontsize=8)
fbox(ax, 0.7, 1.2, 5.7, 0.9, '#566573', "POSIX stdio  /  Terminal I/O", fontsize=9)
...
for i, (t, c) in enumerate(zip(tasks, colors_fw)):
    fbox(ax, 9.6, 5.8 - i*1.1, 2.8, 0.85, c, t, fontsize=7.5)
fbox(ax, 12.7, 4.5, 2.6, 2.1, '#1a5276', "FreeRTOS\nQueues\nxSensorQueue\nxTelemetryQueue", fontsize=7.5)
fbox(ax, 12.7, 3.0, 2.6, 1.2, '#117a65', "Event Group\nxHeartbeatEvents", fontsize=8)
fbox(ax, 9.6, 1.2, 5.7, 0.9, '#566573', "ARM Semihosting  /  QEMU Console", fontsize=9)

# Center concept arrows
ax.annotate('', xy=(9.1, 5.0), xytext=(6.8, 5.0),
            arrowprops=dict(arrowstyle='<->', color='#7f8c8d', lw=2))
ax.text(7.95, 5.15, "Shared\nConcepts", ha='center', va='bottom', fontsize=8, color='#7f8c8d')
...
for i, (bar, t) in enumerate(zip(bars, tasks_data)):
    ax.text(bar.get_width() + 0.05, bar.get_y() + bar.get_height()/2,
            f"Stack: {t[4]} words", va='center', fontsize=9)
ax.set_yticks(y)
ax.set_yticklabels(names, fontsize=10)
ax.set_xlabel("FreeRTOS Priority Level", fontsize=11)
ax.set_title("FreeRTOS Task Priority Chart", fontsize=13, fontweight='bold')
ax.set_xlim(0, 6)
...
ax.set_title("FreeRTOS IPC — Inter-Task Communication", fontsize=13, fontweight='bold', pad=15)
...
ax.text(1.7, 3.6, "100ms\nkick", ha='right', va='center', fontsize=7.5, color='#566573')
...
ax.text(10.6, 4.0, "protects\nUART", ha='left', fontsize=7.5, color='#27ae60')
...
fig.suptitle("STM32F100RB Memory Map", fontsize=13, fontweight='bold')
...
ax.text(used_flash/2, 0, '20.8 KB\n.text', ha='center', va='center', fontsize=9, color='white', fontweight='bold')
ax.text(used_flash + free_flash/2, 0, f'{free_flash:.1f} KB free', ha='center', va='center', fontsize=9, color='#555')
...
for label, size, color in items:
    ...
    ax.text(left + size/2, 0, f'{size:.1f}', ha='center', va='center', fontsize=8, color='white', fontweight='bold')
    left += size
ax.barh(['RAM'], [free_ram], left=[left], color='#bdc3c7', label=f'Free ({free_ram:.1f} KB)')
ax.text(left + free_ram/2, 0, f'{free_ram:.1f} KB', ha='center', va='center', fontsize=9, color='#555')
ax.set_xlabel("Size (KB)")
